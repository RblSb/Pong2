...
class Screen {
	...
	public static var onGamepadButtonGlobal:(id:Int, value:Float)->Void;
	public static var gamepadDeadZone = 0.2;
	static final fps = new Fps();
	static var taskId = -1;
	static var isInited = false;
	static var gamepads:Array<Bool> = [
		for (i in 0...4) false
	];
	public var scale(default, null) = 1.0;
	public final keys:Map<KeyCode, Bool> = [];
	...
	public static function init(?sets:ScreenSets):Void {
		...
		setDefaultScale(sets.defaultScale);
	...
	final function _onResize():Void {
		w = Std.int(System.windowWidth() / scale);
		h = Std.int(System.windowHeight() / scale);
		onResize();
	}

	final function _onRender(framebuffers:Array<Framebuffer>):Void {
	...
	final function _onKeyDown(key:KeyCode):Void {
		keys[key] = true;
		onKeyDown(key);
	}

	final function _onKeyUp(key:KeyCode):Void {
		keys[key] = false;
		onKeyUp(key);
	}

	final function _onMouseDown(button:Int, x:Int, y:Int):Void {
		x = Std.int(x / scale);
		y = Std.int(y / scale);
	...
	final function _onMouseMove(x:Int, y:Int, mx:Int, my:Int):Void {
		x = Std.int(x / scale);
		y = Std.int(y / scale);
		pointers[0].scale = scale;
		pointers[0].x = x;
	...
	final function _onMouseUp(button:Int, x:Int, y:Int):Void {
	...
	final function _onTouchDown(id:Int, x:Int, y:Int):Void {
		if (id > 9) return;
		x = Std.int(x / scale);
		y = Std.int(y / scale);
	...
	final function _onTouchMove(id:Int, x:Int, y:Int):Void {
		if (id > 9) return;
		x = Std.int(x / scale);
		y = Std.int(y / scale);
		pointers[id].scale = scale;
	...
	final function _onTouchUp(id:Int, x:Int, y:Int):Void {
		if (id > 9) return;
		x = Std.int(x / scale);
		y = Std.int(y / scale);
	...
	static function _onGamepadButton(id:Int, value:Float):Void {
		if (onGamepadButtonGlobal != null) {
			onGamepadButtonGlobal(id, value);
		} else if (screen != null) screen.onGamepadButton(id, value);
	}
	...
	public function onGamepadButton(id:Int, value:Float):Void {
		...
		if (value < dz) value = 0;
		if (value > 1 - dz) value = 1;
		inline function keyState(value:Float, key:KeyCode):Void {
			keys[key] = value > 0;
			value > 0 ? onKeyDown(key) : onKeyUp(key);
		}
		switch (id) {
			case 0: // Down (B)
			case 1: // Right (A)
	...
	/**
		Sets the scale of the screen and change screen size.
		Automatically sets this value through `g2.transformation` before `onRender`.
		Calls `onRescale` and then `onResize`.
	**/
	public final function setScale(scale:Float):Void {
	...
	public final function setScaleSilent(scale:Float):Void {
		for (p in pointers) {
			if (p.isActive) {
	...
	**/
	public final function setGraphicScale(g:Graphics, scale:Float):Void {
		g.transformation.setFrom(FastMatrix3.scale(scale, scale));
		w = Std.int(System.windowWidth() / scale);
		h = Std.int(System.windowHeight() / scale);
	}
	...
	public function onRescale(scale:Float):Void {}
	public function onResize():Void {}
	public function onUpdate():Void {}
	public function onRender(frame:Canvas):Void {}
	...
	public function onMouseLock():Void {}
	public function onMouseLockError():Void {}
